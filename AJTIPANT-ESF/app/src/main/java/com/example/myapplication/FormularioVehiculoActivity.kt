...
class FormularioVehiculoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        vehiculoService = retrofit.create(VehiculoService::class.java)
    ...
    private fun configurarModoEdicion() {
        ...
        if (modoEdicion && vehiculoOriginal != null) {
            verificarDisponibilidadVehiculo(vehiculoOriginal!!.id)
        }
    }
    ...
    private fun verificarDisponibilidadVehiculo(id: Int) {
        vehiculoService.isVehiculoDisponible(id).enqueue(object : Callback<Boolean> {
            override fun onResponse(call: Call<Boolean>, response: Response<Boolean>) {
                if (response.isSuccessful && response.body() == false) {
                    Toast.makeText(this@FormularioVehiculoActivity, "Imposible editar Vehiculo", Toast.LENGTH_SHORT).show()
                    finish()
                }
            }

            override fun onFailure(call: Call<Boolean>, t: Throwable) {
                Toast.makeText(this@FormularioVehiculoActivity, "Error al verificar disponibilidad", Toast.LENGTH_SHORT).show()
                finish()
            }
        })
}
