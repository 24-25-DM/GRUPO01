...
import com.example.myapplication.SendMailTask



class VehiculoRepository(private val dbHelper: DatabaseHelper) {

    fun insertVehiculo(vehiculo: Vehiculo, usuarioId: Int, context: Context): Long {
        ...
        val result = db.insert(DatabaseHelper.TABLE_VEHICULO, null, values)

        // Enviar correo después de insertar el vehículo
        enviarCorreo(context, vehiculo)

        return result
    }

    private fun enviarCorreo(context: Context, vehiculo: Vehiculo) {
        val email = "levalladares@uce.edu.ec"
        val subject = "Nuevo Vehículo Insertado"
        val message = """
            Se ha insertado un nuevo vehículo con los siguientes datos:
            ID: ${vehiculo.id}
            Placa: ${vehiculo.placa}
            Marca: ${vehiculo.marca}
            Fecha de Fabricación: ${vehiculo.fechaFabricacion}
            Color: ${vehiculo.color}
            Precio: ${vehiculo.precio}
            Disponible: ${vehiculo.disponible}
            Usuario ID: ${vehiculo.usuarioId}
        """.trimIndent()

        SendMailTask(context, email, subject, message).execute()
    }
