...
class VehiculoRepository(private val dbHelper: DatabaseHelper) {
    ...
    fun insertVehiculo(vehiculo: Vehiculo, usuarioId: Int): Long {
        ...
        val id = db.insert(DatabaseHelper.TABLE_VEHICULO, null, values)
        if (id != -1L) {
            LogRepository(dbHelper).addLogEntry(
                LogRepository.ACTION_CREATE,
                usuarioId,
                "Vehículo creado: ${vehiculo.marca} ${vehiculo.placa}"
            )
        }
        return id
    }
    ...
    fun updateVehicle(vehiculo: Vehiculo, usuarioId: Int): Int {
        ...
        if (rowsAffected > 0) {
            LogRepository(dbHelper).addLogEntry(
                LogRepository.ACTION_UPDATE,
                usuarioId,
                "Vehículo actualizado: ${vehiculo.marca} ${vehiculo.placa}"
            )
        }
        return rowsAffected
    }
    ...
    fun deleteVehicle(vehiculoId: String, usuarioId: Int): Int {
        ...
        if (rowsAffected > 0) {
            LogRepository(dbHelper).addLogEntry(
                LogRepository.ACTION_DELETE,
                usuarioId,
                "Vehículo eliminado: ID $vehiculoId"
            )
        }
        return rowsAffected
    }
