...
class UsuarioRepository(private val dbHelper: DatabaseHelper) {
    ...
    fun validateUser(username: String, password: String): Usuario? {
        ...
        return cursor.use {
            if (it.moveToFirst()) {
                Usuario(
                    id = it.getInt(it.getColumnIndexOrThrow(DatabaseHelper.COLUMN_USUARIO_ID)),
    ...
    fun addUser(nombreUsuario: String, contrasenia: String): Boolean {
        ...
        val nextId = dbHelper.getNextUserId()
        ...
        if (result != -1L) {
            ...
            LogRepository(dbHelper).addLogEntry(LogRepository.ACTION_CREATE, newUser.id, "Nuevo usuario registrado")
            return true
        }
        return false
    }
