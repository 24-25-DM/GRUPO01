...
class DatabaseHelper(context: Context) :
    ...
    fun getNextVehicleId(): Int {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT MAX($COLUMN_VEHICULO_ID) FROM $TABLE_VEHICULO", null)
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) {
            // Agregar la columna de último ingreso a la tabla de usuarios
            db?.execSQL("ALTER TABLE $TABLE_USUARIO ADD COLUMN $COLUMN_USUARIO_ULTIMO_INGRESO TEXT")
        }
    }
    ...
    fun getAllUsuarios(): List<Usuario> {
        ...
        while (cursor.moveToNext()) {
            ...
            val contrasenia = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_USUARIO_CONTRASENIA))
    ...
    fun getUltimoIngreso(userId: Int): String? {
        val db = readableDatabase
        val cursor = db.query(
            TABLE_USUARIO,
            arrayOf(COLUMN_USUARIO_ULTIMO_INGRESO),
            "$COLUMN_USUARIO_ID = ?",
        ...
        var ultimoIngreso: String? = null

        if (cursor.moveToFirst()) {
            val columnIndex = cursor.getColumnIndex(COLUMN_USUARIO_ULTIMO_INGRESO)
            if (columnIndex != -1 && !cursor.isNull(columnIndex)) {
                ultimoIngreso = cursor.getString(columnIndex)
            }
        }
        cursor.close()
    ...
    fun actualizarUltimoIngreso(userId: Int) {
        val db = writableDatabase
        val dateFormat = SimpleDateFormat("dd/MM/yyyy HH:mm:ss", Locale.getDefault())
        val fechaActual = dateFormat.format(Date())

        val values = ContentValues().apply {
            put(COLUMN_USUARIO_ULTIMO_INGRESO, fechaActual)
        }
        ...
        cursor.use {
            ...
            while (it.moveToNext()) {
                vehiculos.add(Vehiculo(
                    ...
                    fechaFabricacion = dateFormat.parse(it.getString(it.getColumnIndexOrThrow(COLUMN_VEHICULO_FECHA_FABRICACION))) ?: Date(),
                    ...
                    imageResource = it.getInt(it.getColumnIndexOrThrow(COLUMN_VEHICULO_IMAGEN)).toString(),
    ...
    fun getVehiculosByUsuario(usuarioId: Int): List<Vehiculo> {
        ...
        while (cursor.moveToNext()) {
            ...
            val fechaFabricacionStr = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_VEHICULO_FECHA_FABRICACION))
            val fechaFabricacion = dateFormat.parse(fechaFabricacionStr) // Convertir texto a Date
            ...
            val disponible = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_VEHICULO_DISPONIBLE)) == 1
            val imageResource = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_VEHICULO_IMAGEN))

            vehiculos.add(Vehiculo(id, placa, marca, fechaFabricacion, color, precio, disponible, imageResource, usuarioId))
        }
        cursor.close()
        return vehiculos
    }
